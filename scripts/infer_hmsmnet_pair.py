#!/usr/bin/env python3
"""Run single-pair inference with the HMSMNet host baseline used for MDRF evaluation."""
...
from hmsmnet import HMSMNet  # noqa: E402
...
def central_gradients(image: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
def main() -> None:
    ...
    gx_np, gy_np = central_gradients(left_np)
    height, width = left_np.shape

    model = HMSMNet(height=height, width=width, channel=1, min_disp=args.min_disp, max_disp=args.max_disp)
    checkpoint = torch.load(args.checkpoint, map_location=device)
    model.load_state_dict(clean_state_dict(checkpoint), strict=not args.non_strict)
    model.to(device).eval()

    with torch.no_grad():
        _, _, _, prediction = model(
            to_tensor(left_np, device),
            to_tensor(right_np, device),
            to_tensor(gx_np, device),
            to_tensor(gy_np, device),
        )
